...
class AzureClientCredential:
    @staticmethod
    def get_credential(
        env_config: Type[EnvConfig] = EnvConfig,
...
def default_token_provider(
    credential: type[AzureClientCredential] = AzureClientCredential,
    env_config: Type[EnvConfig] = EnvConfig,
) -> str:
    return (
        credential.get_credential(env_config)
        .get_token(env_config.AZURE_URL_TOKEN)
        .token
    )
